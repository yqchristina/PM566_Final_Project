...
### Merging the protein and microarray datasets 

```{r}
combined <- merge(cyp450,microarray, by.x = "Gene_name", by.y = "gene-symbol", all.x = TRUE, all.y = FALSE)
```
...
To examine the overall expression of the CYP450s, the average expression in different brain areas for all the donors was calculated. The genes with equal or higher expression than CYP11A1 was filtered out.
```{r}
...
Figure 4: RNA Expression of CYP450s with Equal of Higher Expression than CYP11A1. There are `r length(unique(averages_higher$gene_name))` CYP450 enzymes with equal or higher expression than CYP11A1 in the brain. As seen from the graph, CYP51A1 appears to have the highest expression out of all the CYP450s, followed by CYP4Z1 and CYP2J2. 


Now combining the CYP450s more highly expressed in the brain than CYP11A1 with the UniProt data to see which ones have cholesterol related function. 
```{r}
...
Figure 5: Subcellular localization of CYP450s that have cholesterol related functions with equal or higher expression in the brain than CYP11A1. 

We see that there are `r length(unique(combined_chol$Gene_name))` CYP450s with cholesterol related function when the two datasets are combined together. The CYP450s are listed below.
